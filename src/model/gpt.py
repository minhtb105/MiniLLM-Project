...
class GPTModel(Module):
    ...
    def forward(self, input_ids: np.ndarray) -> Tensor:
        ...
        input_ids: numpy array shape (B, L) of token ids
        returns: Tensor of shape (B, L, vocab_size)
        """
        # Convert input ids to embeddings via model's Embedding (which returns Tensor)
        x = self.token_emb(input_ids)  # expects token_ids list/ndarray -> returns Tensor (B,L,D)
    ...
    def generate(
        ...
        """
        beams = [(input_ids, 0.0)]  # list of (token_ids, score)
        finished = []
        ...
        for _ in range(max_new_tokens):
            ...
            for seq, log_prob in beams:
                ...
                logits = self(seq)[0, -1]  # (vocab_size,)
                ...
                cummulative_probs = np.cumsum(sorted_probs)
                ...
                next_log_prob = np.log(probs[next_token_ids] + 1e-8)
                
                # update beam sequences
                new_seq = np.concatenate([seq, [next_token_ids]])
                new_beams.append((new_seq, log_prob + next_log_prob))
            ...
            for seq, _ in beams:
                if seq[-1] == self.eos_token_id:  # cần định nghĩa trước
                    finished.append(seq)
